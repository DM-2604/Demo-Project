...
import { Ksdashboardtile } from '@ks_dashboard_ninja/js/ks_dashboard_tile';
import { Ksdashboardlistview } from '@ks_dashboard_ninja/js/ks_dashboard_list';
import { Ksdashboardtodo } from '@ks_dashboard_ninja/js/ks_dashboard_to_do';
import { Ksdashboardkpiview } from '@ks_dashboard_ninja/js/ks_dashboard_kpi';
import { Ksdashboardgraph } from '@ks_dashboard_ninja/js/ks_dashboard_graphs';
...
//        var items = self.ksSortItems(self.ks_dashboard_data.ks_item_data)
    ...
    ks_fetch_items_data(){
        ...
        self.ks_dashboard_data.ks_dashboard_items_ids.forEach(function(item_id){
            items_promises.push(rpc("/web/dataset/call_kw",{
                model: "ks_dashboard_ninja.board",
...
//                    var end_date = formatDateTime(end_date, self.datetime_format).format("YYYY-MM-DD H:m:s");
    ...
    onChartMoreInfoClick(e) {
            ...
            console.log("PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP",domain)
...
//            this.ks_dashboard_data.ks_selected_board_id = layout_id;
    ...
    _onKsSaveLayoutClick(){
        ...
        if (dashboard_title != false && dashboard_title != 0 && dashboard_title !== self.ks_dashboard_data.name) {
            ...
            var new_model = `/web/dataset/call_kw`
            this._rpc(new_model,{
                model: model,
                method: 'write',
                args: [rec_id, {
   ...
   async speak_once(ev,item){
        ...
        if (!this.ks_speeches.length){
            if ($(ks_audio).find('audio').attr('src') && $(ks_audio).find('audio')[0].paused){
                ...
                this.ks_speeches.push(this._rpc('/web/dataset/call_kw',{
                model : "ks_dashboard_ninja.arti_int",
