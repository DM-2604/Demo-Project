    ...
    _update_view(){
        if(!this.kpi_data[1]){
            if (this.field.ks_target_view === "Progress Bar" && this.field.ks_goal_enable) {
                $('#' + this.item.id).find('#ks_progressbar').val(parseInt(this.state.item_info_kpi1.target_progress_deviation));
            }
            ...
            if(this.field.ks_data_comparison == 'Percentage'){
                ...
                if (this.field.ks_target_view === "Progress Bar") {
                    if (this.state.item_info_kpi2.count) $('#' + this.item.id).find('#ks_progressbar').val(parseInt(this.state.item_info_kpi2.count));
...
//            parseInt(Math.round((count_1 / target_1) * 100)) ? formatInteger(Math.round((count_1 / target_1) * 100)) : "0"
        ...
        var item_info = {
            item: this.item,
            id: field.id,
            count_1: globalfunction.ksNumFormatter(count_1, 1),
            count_1_tooltip: count_1,
            count_2: kpi_data[1] ? String(count_2) : false,
        ...
        if (field.ks_multiplier_active){
            item_info['count_1'] = globalfunction._onKsGlobalFormatter(count_1 * field.ks_multiplier, field.ks_data_format, field.ks_precision_digits);
            item_info['count_1_tooltip'] = count_1 * field.ks_multiplier
        }else{
            item_info['count_1'] = globalfunction._onKsGlobalFormatter(count_1, field.ks_data_format, field.ks_precision_digits);
        }
         if (target_1){
            item_info['target'] = globalfunction._onKsGlobalFormatter(target_1, field.ks_data_format, field.ks_precision_digits);
        }
        if (field.ks_unit){
            if (field.ks_multiplier_active){
                var ks_record_count = count_1 * field.ks_multiplier
            }else{
                var ks_record_count = count_1
            }
            var ks_selection = field.ks_unit_selection;
            if (ks_selection === 'monetary') {
