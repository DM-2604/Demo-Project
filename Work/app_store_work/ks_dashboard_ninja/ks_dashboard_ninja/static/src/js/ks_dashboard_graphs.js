...
//            this.chart_container[item.id] = chart;
            ...
            $ks_gridstack_container.find('.ks_chart_card_body').append($("<div class='graph_text'>").text("No Data Available."));
                ...
                if (item.ks_dashboard_item_type === 'ks_heading') {
                    $ks_gridstack_container.find('.ks_chart_card_body').append(
                        $("<div class='graph_text'>").text(item.name)
                    );
                } else {
                    $ks_gridstack_container.find('.ks_chart_card_body').append(
                        $("<div class='graph_text'>").text("No Data Available.")
                    );
                }
            }
    }
...
//                domain = item_data['domains'][sequence+1] ? item_data['domains'][sequence+1] : []
                domain = item_data['domains'][1] ? item_data['domains'][1] : []
//                alert(domain.toString())
                alert(domain)
...
//                            $($(".ks_dashboard_main_content").find(".grid-stack-item[gs-id=" + item_id + "]").children()[0]).find(id_name).addClass('d-none');
                    ...
                    self.ksFetchChartItem(item_id, domain)
//                        $(self.$el.find('.ks_pager')).removeClass('d-none');
    ...
    ksFetchChartItem(id, domain=[]) {
        ...
        return self._rpc("/web/dataset/call_kw",{
            ...
            method: 'ks_fetch_item',
            args: [
                [item_data.id], self.ks_dashboard_data.ks_dashboard_id, {'domain':domain}
            ],
            kwargs:{},
        }).then(function(new_item_data) {
            console.log(new_item_data.toString())
