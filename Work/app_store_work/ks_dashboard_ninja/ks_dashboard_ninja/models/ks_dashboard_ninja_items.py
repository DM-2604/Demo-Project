...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_record_field = fields.Many2one('ir.model.fields',
                                      domain="[('model_id','=',ks_model_id), ('name','!=','id'), ('store','=',True), ('ttype','=','monetary')]",
                                      string="Record Field")
    ...
    ks_chart_measure_field = fields.Many2many(
        'ir.model.fields',
        'ks_dn_measure_field_rel',
        'measure_field_id',
        'field_id',
        domain="[('model_id','=',ks_model_id), ('name','!=','id'), ('name','!=','sequence'), ('store','=',True), ('ttype','=','monetary')]",
        string="Measure 1",
        help='Data points to be selected.'
    )
    ...
    def layout_four_font_change(self):
        if self.ks_dashboard_item_theme != "white":
            ...
            if self.ks_layout == 'layout4':
                ...
                self.ks_background_color = "pink,0.99"
                self.ks_font_color = "#000000,0.99"
                self.ks_default_icon_color = "#000000,0.99"
    ...
    def ks_convert_into_proper_domain(self, ks_domain, rec, domain=[]):
        ...
        if rec.ks_date_filter_field:
            if not rec.ks_date_filter_selection or rec.ks_date_filter_selection == "l_none":
                ...
                if selected_end_date and not selected_start_date:
                    ks_date_domain = [(rec.ks_date_filter_field.name, "<=",
                ...
                if selected_start_date and selected_end_date:
                    ...
                    if rec.ks_year_period and rec.ks_year_period != 0 and rec.ks_dashboard_item_type:
                        ...
                        ks_date_domain = ['&',
                                          (date_field_name, ">=", fields.datetime.strftime(selected_start_date,
        ...
        # ✅ FIX: Ensure ks_domain is a string before using safe_eval
        if isinstance(ks_domain, list):
            proper_domain = ks_domain
        elif isinstance(ks_domain, str):
            proper_domain = safe_eval(ks_domain) if ks_domain.strip() else []
        else:
            proper_domain = []

        # Extend with dynamic domain components
        if ks_date_domain:
    ...
    def ks_get_chart_data(self):
        print("dddddddd===================dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd")
        for rec in self:
    ...
    def _ks_get_chart_data(self, domain=[]):
        ...
        if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type != 'ks_tile' and \
            ...
            print("KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK",ks_chart_data)
            ...
            if rec.ks_unit and rec.ks_unit_selection == 'monetary':
                ...
                print("11111111111111111111111111111111111111111111111111", ks_chart_data)
            ...
            if rec.ks_chart_data_count_type == "count":
                print("22222222222222222222222222222222222222222222222", ks_chart_data)
                rec.ks_chart_measure_field = False
                rec.ks_chart_measure_field_2 = False
                ...
                print("444444444444444444444444444444444444444444444444444", ks_chart_data)
                if rec.ks_dashboard_item_type == 'ks_bar_chart':
                    print("3333333333333333333333333333333333333333333333333333333", ks_chart_data)
                    if rec.ks_chart_measure_field_2:
                ...
                for res in range(0, len(rec.ks_chart_measure_field)):
                    print("555555555555555555555555555555555555555555555555555555", ks_chart_data)
                    if rec.ks_chart_data_count_type == 'sum':
                        ks_data_count_type = 'sum'
                    elif rec.ks_chart_data_count_type == 'average':
                        ks_data_count_type = 'avg'
                    ...
                    if rec.ks_chart_cumulative_field and res < len(rec.ks_chart_cumulative_field)  and \
                        ...
                        print("66666666666666666666666666666666666666666666666666666666", ks_chart_data)
                        ks_chart_data['datasets'].append(
            ...
            if rec.ks_chart_data_count_type == "count" and not self.ks_fill_temporal and not rec.ks_sort_by_field:
                orderby = 'count'
                print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", ks_chart_data)
            else:
            ...
            if ((rec.ks_chart_data_count_type != "count" and ks_chart_measure_field) or (
                ...
                print("BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB", ks_chart_data)
                ...
                if (rec.ks_chart_groupby_type == 'date_type' and rec.ks_chart_date_groupby) or \
                    ...
                    if rec.ks_chart_groupby_type == 'date_type' and rec.ks_goal_enable and rec.ks_dashboard_item_type in [
                        ...
                        if rec.ks_goal_lines and len(rec.ks_goal_lines) != 0:
                            ...
                            domains = {}
                            print("DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD",domains)
                            counter = 0
                            ...
                            print("cccccccccccccccccccccccccccccccccccccccccccccccccccccccc", ks_chart_data['domains'])
                            ks_chart_data['domains'].clear()
                            print("AFFFAFFFFFFFFFFFFAFFFFFFAAAAAAAAAAAFFFFFFFAAAAAAAAAAAAAAAA", ks_chart_data['domains'])
                            datasets = []
                ...
                if len(ks_chart_measure_field) != 0 or rec.ks_chart_data_count_type == 'count':
                    ...
                    ks_chart_data = {
                        'labels': [],
                        'datasets': [],
                        'domains': [["&", "&", ["user_id", "in", [2]], ["name", "=", "Renovations"], ["task_properties_definition", "=", False]], ["&", "&", ["user_id", "in", [2]], ["name", "=", "Research & Development"], ["task_properties_definition", "=", False]]],
                        'ks_selection': "",
                        'ks_currency': 0,
                        'ks_field': "",
                        'previous_domain': [["user_id", "in", [2]]]
                    }
        ...
        for res in ks_chart_records:
            ...
            if all(measure_field in res for measure_field in ks_chart_measure_field):
                if ks_chart_groupby_type == "relational_type":
                    ...
                    label = res[ks_chart_groupby_field]
                print("fnsdlkacsdl", res.get('__domain', []))
    ...
    def ks_fetch_drill_down_data(self, item_id, domain, sequence):
        ...
        print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAaaaa",domain)
        ...
        if action_line.ks_chart_type == 'ks_list_view':
            ...
            if action_line.ks_item_action_field.ttype == 'many2one':
                ...
                ks_list_view_records = self.env[record.ks_model_name] \
                    .read_group(domain, ks_list_fields, [action_line.ks_item_action_field.name], orderby=orderby,
                                limit=limit, lazy=False)
                print("ks_list_view_records===>", ks_list_view_records)
                for res in ks_list_view_records:
                    if all(list_fields in res for list_fields in ks_list_fields):
                        counter = 0
                        ...
                        print("data_row data_row =========================",data_row)
...
class KsDashboardItemsActions(models.Model):
    ...
    ks_action_item_name = fields.Char(string = "Action item name")
